...
int number=0;//frame number for filenames
  ...
  //width, height, and bitcount are the key factors:
    int32_t widthin = 1200;
    int32_t heightin = 700;
    int32_t widthin2 = 1000;
    int32_t heightin2 = 667;
    int32_t widthout = 1200;
    int32_t heightout = 700;
...
void drawGridtransparent(int border,int number,double transparency){
    int size=(heightin -border*2)/number;
    for (int j=100;j<=heightin-100;j+=size){
        for (int i=100;i<widthin-100;i++){
            for (int m=0;m<3;m++){
                for (int k=0;k<4;k++){
                screen [i][j+k-2][m]=screen [i][j][m]*transparency/100.0f+screen [i][j+k-2][m]*(1-transparency/100.0f);
                }
    ...
    for (int i=100;i<=widthin-100;i+=size){
        for (int j=100;j<heightin-100;j++){
            for (int m=0;m<3;m++){
                for (int k=0;k<4;k++)
                screen [i+k-2][j][m]=screen [i][j][m]*transparency/100.0f+screen [i+k-2][j][m]*(1-transparency/100.0f);
...
for (int i=0;i<100;i++){
    noise(60,30);
number++;
drawGridtransparent(100,10,number);
...
for (int i=0;i<200;i++){
    noise(60,30);
    number++;
    double trans = (200 - number);
    drawGridtransparent(100,10,trans);
    sprintf(filestring,"frame%d.bmp",number);
    puts(filename);
    outputbmp(filestring);
